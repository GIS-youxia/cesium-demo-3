...
class Titleset {
  ...
  async showOne(id) {
    const res = await getOneTitleset(id)
  ...
  addOne3dTitleset(ele, index, isConfig = false) {
    const _this = this
  ...
  tileVisible_addEventListener_fun(cesium3DTile) {
    const v_this = this
    const ele = v_this.curEleDatas[v_this.index]
    // 以下设置白膜的打光效果
  ...
  makeEffect(tileset, ele, index, isConfig) {
    const _this = this
  ...
  update3dtilesMaxtrix(tileset, ele) {
    if (!tileset.ready) {
      return
    }
  ...
  addtest() {
    ...
    this.viewer.scene.primitives
      ...
      .readyPromise.then(function(tileset) {
        ...
        tileset.tileVisible.addEventListener(function(cesium3DTile) {
          // 以下设置白膜的打光效果
          const cesium3DTileCon = cesium3DTile.content
          const featuresLength = cesium3DTileCon.featuresLength
          for (let i = 0; i < featuresLength; i += 2) {
